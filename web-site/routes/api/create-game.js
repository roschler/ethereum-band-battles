/**
 * This is the route for the page that lets someone start a game.
 */
...
router.get('/create-game',function(req,res, next){
    try
    {
		...
		res.render('create-game', locals , function (err, html)
		{
			if (err)
			{
    ...
    {
        console.log('[ERROR: create-game] Details -> ' + err.message);
        res.status(http_status_codes.INTERNAL_SERVER_ERROR).send('Error during game creation.');
        return;
    } // try/catch
