...
// NOTE: This version has the changes made to mark payable addresses as payable.

// Use these when reverting to non-proxied contract for trace debugging with Truffle console.
import "../node_modules/openzeppelin-solidity/contracts/math/SafeMath.sol";
import "../node_modules/openzeppelin-solidity/contracts/lifecycle/Pausable.sol";

// Use these in production to take advantage of Open Zeppelin's ugradeable contract technology.
...
// contract EtherBandBattlesManager{
    ...
    */
    ...
    struct structGame {
        ...
        address payable gameCreatorAddr;
     ...
     */
    function addPlayer(uint256 _gameId, address payable _playerAddr, bytes32 _videoId)
            public payable
     ...
     * @param _bandAddr - The address of the band the video belongs to.  May be 0
     *  if we don't have it yet.'
     *
     ...
     */
    function addGameRoundResult(uint256 _gameId, uint256 _roundNum, bytes32 _videoId, bytes32 _ownerId, address payable _winnerAddr, address payable _bandAddr)
            public
    ...
    function addAddressToPendingPaymentsList(
     ...
     /** @notice This function adds an address to the pending payment linked list.
     *
     * @param _mapKey - The key to the pending payment record.
     */
     ...
     *
     * @dev The game ID will be 0 for escrow payments since we don't track those by game.  An escrow payment
     *  contains the total sum payments due to the escrow payee.
     *
    ...
    function processBandPayments(
        ...
        for (uint i = 0; i < s_mapGameIdToGameResults[_gameId].length; i++)
        {
            ...
            address payable bandAddr = s_mapGameIdToGameResults[_gameId][i].bandAddr;
    ...
    function processPlayerPayments(
        ...
        for (uint i = 0; i < s_mapGameIdToGameResults[_gameId].length; i++)
        {
            address payable payeeAddr = s_mapGameIdToGameResults[_gameId][i].winnerAddr;
    ...
    function finalizeGame(
        ...
        address payable houseAddr = s_mapGameIdToGame[_gameId].houseAddr;
    ...
    function addNewGame(
            ...
            address payable _gameCreatorAddr,
     ...
     */
    function getIsPlayerInGame(uint256 _gameId, address payable _playerAddr)
            public view
            onlyIfValidGameId(_gameId)
            returns (bool) {
     ...
     */
    function getPlayerBalance(uint256 _gameId, address payable _payeeAddr)
            public view
            onlyIfValidGameId(_gameId)
            returns (uint256) {
    ...
    function getNextPendingPaymentDetails(uint256 _paymentNdx)
            public view
            returns(uint256, uint256, bytes32, address payable, uint256, uint) {
    ...
    */
     ...
     */
    function setHouseAddress(uint256 _gameId, address payable _houseAddr) public
